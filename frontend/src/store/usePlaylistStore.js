...
export const usePlaylistStore = create((set, get) => ({
  ...
  createPlaylist: async (playlistData) => {
    try {
      set({ isLoading: true });
      const response = await exiosInstance.post(
  ...
  getAllPlaylists: async () => {
    try {
      set({ isLoading: true });
      const response = await exiosInstance.get("/playlist");
  ...
  getPlaylistDetails: async (playlistId) => {
    try {
      set({ isLoading: true });
      const response = await exiosInstance.get(`/playlist/${playlistId}`);
  ...
  addProblemToPlaylist: async (playlistId, problemIds) => {
    try {
      set({ isLoading: true });
      await exiosInstance.post(`/playlist/${playlistId}/add-problem`, {
        problemIds,
      });
  ...
  removeProblemFromPlaylist: async (playlistId, problemIds) => {
    try {
      set({ isLoading: true });
      await exiosInstance.post(`/playlist/${playlistId}/remove-problems`, {
        problemIds,
      });
  ...
  deletePlaylist: async (playlistId) => {
    try {
      set({ isLoading: true });
      await exiosInstance.delete(`/playlist/${playlistId}`);
